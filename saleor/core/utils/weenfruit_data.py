...
def create_products_by_schema(placeholder_dir, create_images):
    path = os.path.join(
        settings.PROJECT_ROOT, "saleor", "static", "populatedb_weenfruit_data.json"
    )
    with open(path) as f:
    ...
    data_pages = {
        1: {
            ...
            "content": {
                "blocks": [
                    {
                        "data": {"text": "Trái Cây MêKông", "level": 2},
                        "type": "header",
                    },
                    {
                        "data": {
                            "text": (
                                "Website chuyên bán sỉ và lẻ sầu riêng, bưởi da xanh "
                                "và các loại trái cây bốn mùa như Sapôchê, Thanh long ruột đỏ,... v.v."
                            ),
                            "level": 2,
                        },
                        "type": "header",
                    ...
                    {"data": {"text": ""}, "type": "paragraph"},
                    {
                        "data": {
                            "text": (
                                '<a href="https://github.com/thinhhuynh/saleor">'
                                "Phát triển dựa trên nền tảng Saleor!</a>"
                            )
                        },
                        "type": "paragraph",
                    },
...
def create_menus():
    ...
    item, _ = bottom_menu.items.get_or_create(name="Trái cây đặc biệt", collection=collection)
    ...
    item_saleor = bottom_menu.items.get_or_create(name="Trái Cây MêKông", url="/")[0]
    ...
    item_saleor.children.get_or_create(
        name="Khuyến mãi", url=api_url, menu=bottom_menu
    )

    # item_saleor = bottom_menu.items.get_or_create(name="Đặt hàng", url="/")[0]
    # item_saleor.children.get_or_create(
    #     name="Tạo tài khoản", url=api_url, menu=bottom_menu
    # )
    # item_saleor.children.get_or_create(
    #     name="Giao hàng", url=api_url, menu=bottom_menu
    # )
    # item_saleor.children.get_or_create(
    #     name="Đổi-trả hàng", url=api_url, menu=bottom_menu
    # )
    # item_saleor.children.get_or_create(
    #     name="Thanh toán", url=api_url, menu=bottom_menu
    # )
    # item_saleor = bottom_menu.items.get_or_create(name="Hỗ trợ", url="/")[0]
    # item_saleor.children.get_or_create(
    #     name="Câu hỏi thường gặp", url=api_url, menu=bottom_menu
    # )
    # item_saleor.children.get_or_create(
    #     name="Cách tính tiền", url=api_url, menu=bottom_menu
    # )
    # item_saleor.children.get_or_create(
    #     name="Liên lạc", url=api_url, menu=bottom_menu
    # )
    # item_saleor = bottom_menu.items.get_or_create(name="Mạng xã hội", url="/")[0]
    # item_saleor.children.get_or_create(
    #     name="Facebook", url=api_url, menu=bottom_menu
    # )
    # item_saleor.children.get_or_create(
    #     name="Youbute", url=api_url, menu=bottom_menu
    # )
    # item_saleor = bottom_menu.items.get_or_create(name="Thiết lập", url="/")[0]
    # item_saleor.children.get_or_create(
    #     name="Ngôn ngữ", url=api_url, menu=bottom_menu
    # )
