...
ROOT_URLCONF = "weenspace.urls"

WSGI_APPLICATION = "weenspace.wsgi.application"
...
DATABASE_ROUTERS = ["weenspace.core.db_routers.PrimaryReplicaRouter"]
...
STATICFILES_DIRS = [
    ("images", os.path.join(PROJECT_ROOT, "weenspace", "static", "images"))
]
STATICFILES_FINDERS = [
...
context_processors = [
    ...
    "weenspace.site.context_processors.site",
]
...
PASSWORD_HASHERS = [
    *global_settings.PASSWORD_HASHERS,
    ...
    "weenspace.core.hashers.SHA512Base64PBKDF2PasswordHasher",
]
...
JWT_MANAGER_PATH = os.environ.get(
    "JWT_MANAGER_PATH", "weenspace.core.jwt_manager.JWTManager"
)

MIDDLEWARE = [
    "django.middleware.security.SecurityMiddleware",
    "django.middleware.common.CommonMiddleware",
    "weenspace.core.middleware.jwt_refresh_token_middleware",
]
...
if ENABLE_RESTRICT_WRITER_MIDDLEWARE:
    MIDDLEWARE = ["weenspace.core.db.connection.log_writer_usage_middleware"] + MIDDLEWARE
...
if ENABLE_DEBUG_TOOLBAR:
    ...
    try:
        ...
        MIDDLEWARE.append("weenspace.graphql.middleware.DebugToolbarMiddleware")
...
LOGGING = {
    ...
    "formatters": {
        ...
        "json": {
            "()": "weenspace.core.logging.JsonFormatter",
        ...
        "celery_json": {
            "()": "weenspace.core.logging.JsonCeleryFormatter",
            "datefmt": "%Y-%m-%dT%H:%M:%SZ",
            "format": (
        ...
        "celery_task_json": {
            "()": "weenspace.core.logging.JsonCeleryTaskFormatter",
            "datefmt": "%Y-%m-%dT%H:%M:%SZ",
            "format": (
                "%(asctime)s %(levelname)s %(celeryTaskId)s %(celeryTaskName)s "
                "%(message)s "
    ...
    "loggers": {
        ...
        "weenspace": {"level": "DEBUG", "propagate": True},
        "weenspace.graphql.errors.handled": {
            "handlers": ["default"],
            "level": "INFO",
            "propagate": False,
        },
...
TEST_RUNNER = "weenspace.tests.runner.PytestTestRunner"
...
if AWS_MEDIA_BUCKET_NAME:
    DEFAULT_FILE_STORAGE = "weenspace.core.storages.S3MediaStorage"
elif GS_MEDIA_BUCKET_NAME:
    DEFAULT_FILE_STORAGE = "weenspace.core.storages.GCSMediaStorage"
elif AZURE_CONTAINER:
    DEFAULT_FILE_STORAGE = "weenspace.core.storages.AzureMediaStorage"
...
AUTHENTICATION_BACKENDS = [
    "weenspace.core.auth_backend.JSONWebTokenBackend",
    "weenspace.core.auth_backend.PluginBackend",
]
...
CELERY_TASK_ROUTES = {
    "weenspace.plugins.webhook.tasks.observability_reporter_task": {
        "queue": "observability"
    },
    "weenspace.plugins.webhook.tasks.observability_send_events": {
        "queue": "observability"
    },
}
...
CELERY_BEAT_SCHEDULE = {
    ...
    "deactivate-preorder-for-variants": {
        "task": "weenspace.product.tasks.deactivate_preorder_for_variants_task",
        "schedule": timedelta(hours=1),
    },
    "delete-expired-reservations": {
        "task": "weenspace.warehouse.tasks.delete_expired_reservations_task",
        "schedule": timedelta(days=1),
    },
    "delete-expired-checkouts": {
        "task": "weenspace.checkout.tasks.delete_expired_checkouts",
        "schedule": crontab(hour=0, minute=0),
    },
    "delete_expired_orders": {
        "task": "weenspace.order.tasks.delete_expired_orders_task",
        "schedule": crontab(hour=2, minute=0),
    },
    "delete-outdated-event-data": {
        "task": "weenspace.core.tasks.delete_event_payloads_task",
        "schedule": timedelta(days=1),
    },
    "deactivate-expired-gift-cards": {
        "task": "weenspace.giftcard.tasks.deactivate_expired_cards_task",
        "schedule": crontab(hour=0, minute=0),
    },
    "update-stocks-quantity-allocated": {
        "task": "weenspace.warehouse.tasks.update_stocks_quantity_allocated_task",
        "schedule": crontab(hour=0, minute=0),
    },
    "delete-old-export-files": {
        "task": "weenspace.csv.tasks.delete_old_export_files",
        "schedule": crontab(hour=1, minute=0),
    },
    "handle-promotion-toggle": {
        "task": "weenspace.discount.tasks.handle_promotion_toggle",
        "schedule": initiated_promotion_webhook_schedule,
    },
    "update-products-search-vectors": {
        "task": "weenspace.product.tasks.update_products_search_vector_task",
    ...
    "update-gift-cards-search-vectors": {
        "task": "weenspace.giftcard.tasks.update_gift_cards_search_vector_task",
        "schedule": timedelta(seconds=BEAT_UPDATE_SEARCH_SEC),
        "options": {"expires": BEAT_UPDATE_SEARCH_EXPIRE_AFTER_SEC},
    },
    "expire-orders": {
        "task": "weenspace.order.tasks.expire_orders_task",
        "schedule": BEAT_EXPIRE_ORDERS_AFTER_TIMEDELTA,
    },
    "remove-apps-marked-as-removed": {
        "task": "weenspace.app.tasks.remove_apps_task",
        "schedule": crontab(hour=3, minute=0),
    },
    "release-funds-for-abandoned-checkouts": {
        "task": "weenspace.payment.tasks.transaction_release_funds_for_checkout_task",
        "schedule": timedelta(minutes=10),
    },
    "recalculate-promotion-rules": {
        "task": (
            "weenspace.product.tasks"
    ...
    "recalculate-discounted-price-for-products": {
        "task": "weenspace.product.tasks.recalculate_discounted_price_for_products_task",
        "schedule": timedelta(seconds=BEAT_PRICE_RECALCULATION_SCHEDULE),
        "options": {"expires": BEAT_PRICE_RECALCULATION_SCHEDULE_EXPIRE_AFTER_SEC},
    },
}
...
if OBSERVABILITY_ACTIVE:
    CELERY_BEAT_SCHEDULE["observability-reporter"] = {
        "task": "weenspace.plugins.webhook.tasks.observability_reporter_task",
...
BUILTIN_PLUGINS = [
    "weenspace.plugins.avatax.plugin.AvataxPlugin",
    "weenspace.plugins.webhook.plugin.WebhookPlugin",
    "weenspace.payment.gateways.dummy.plugin.DummyGatewayPlugin",
    "weenspace.payment.gateways.dummy_credit_card.plugin.DummyCreditCardGatewayPlugin",
    "weenspace.payment.gateways.stripe.deprecated.plugin.DeprecatedStripeGatewayPlugin",
    "weenspace.payment.gateways.stripe.plugin.StripeGatewayPlugin",
    "weenspace.payment.gateways.braintree.plugin.BraintreeGatewayPlugin",
    "weenspace.payment.gateways.razorpay.plugin.RazorpayGatewayPlugin",
    "weenspace.payment.gateways.adyen.plugin.AdyenGatewayPlugin",
    # "weenspace.payment.gateways.authorize_net.plugin.AuthorizeNetGatewayPlugin",
    "weenspace.payment.gateways.np_atobarai.plugin.NPAtobaraiGatewayPlugin",
    "weenspace.plugins.invoicing.plugin.InvoicingPlugin",
    "weenspace.plugins.user_email.plugin.UserEmailPlugin",
    "weenspace.plugins.admin_email.plugin.AdminEmailPlugin",
    "weenspace.plugins.sendgrid.plugin.SendgridEmailPlugin",
    "weenspace.plugins.openid_connect.plugin.OpenIDConnectPlugin",
]

# Plugin discovery
EXTERNAL_PLUGINS = []
installed_plugins = pkg_resources.iter_entry_points("weenspace.plugins")
...
#
# Refer to our guide on https://docs.weenspace.com/docs/next/guides/opentracing-jaeger/.
#
