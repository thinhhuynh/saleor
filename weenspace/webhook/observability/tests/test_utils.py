...
def patch_get_buffer(buffer):
    with patch(
        "weenspace.webhook.observability.utils.get_buffer", return_value=buffer
    ) as get_buffer:
        yield get_buffer
...
def patch_get_webhooks(observability_webhook_data):
    with patch(
        "weenspace.webhook.observability.utils.get_webhooks",
...
@patch("weenspace.webhook.observability.utils.cache.get")
@patch("weenspace.webhook.observability.utils.get_webhooks_for_event")
...
@patch("weenspace.webhook.observability.utils.ApiCall.report")
...
@patch("weenspace.webhook.observability.utils.put_event")
