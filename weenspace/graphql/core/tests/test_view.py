...
def test_graphql_execution_exception(monkeypatch, api_client):
    ...
    monkeypatch.setattr("weenspace.graphql.api.execute", mocked_execute)
...
def test_invalid_query_graphql_errors_are_logged_in_another_logger(
    ...
    assert graphql_log_handler.messages == [
        "weenspace.graphql.errors.handled[INFO].GraphQLError"
    ]
...
def test_invalid_syntax_graphql_errors_are_logged_in_another_logger(
    ...
    assert response.status_code == 400
    assert graphql_log_handler.messages == [
        "weenspace.graphql.errors.handled[INFO].GraphQLSyntaxError"
    ]
...
def test_permission_denied_query_graphql_errors_are_logged_in_another_logger(
    ...
    assert graphql_log_handler.messages == [
        "weenspace.graphql.errors.handled[INFO].PermissionDenied"
    ]
...
@mock.patch("weenspace.graphql.product.schema.resolve_collection_by_id")
def test_unexpected_exceptions_are_logged_in_their_own_logger(
    ...
    assert response.status_code == 200
    assert graphql_log_handler.messages == [
        "weenspace.graphql.errors.unhandled[ERROR].NotImplementedError"
    ]
...
@mock.patch("weenspace.graphql.views.cache.set")
@mock.patch("weenspace.graphql.views.cache.get")
