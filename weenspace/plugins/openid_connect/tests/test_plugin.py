...
def test_external_refresh_from_cookie(
    ...
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.utils.get_decoded_token",
    ...
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.client.OAuth2Client.refresh_token",
    ...
    mocked_refresh_token.assert_called_once_with(
        "https://weenspace.com/oauth/token",
...
def test_external_refresh_with_scope_permissions(
    ...
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.plugin.get_parsed_id_token",
        Mock(return_value=mocked_jwt_validator),
    )
    oauth_payload = {
        "access_token": "FeHkE_QbuU3cYy1a1eQUrCE5jRcUnBK3",
...
@freeze_time("2019-03-18 12:00:00")
@patch("weenspace.plugins.openid_connect.utils.cache.set")
@patch("weenspace.plugins.openid_connect.utils.cache.get")
def test_external_obtain_access_tokens(
    ...
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.client.OAuth2Client.fetch_token",
        mocked_fetch_token,
    )
    ...
    user, _, _ = get_or_create_user_from_payload(
        claims,
        oauth_url="https://weenspace.com/oauth",
    )
...
def test_external_obtain_access_tokens_with_permissions(
    ...
    user, _, _ = get_or_create_user_from_payload(claims, "https://weenspace.com/oauth")
    user.is_staff = True
    expected_tokens = create_tokens_from_oauth_payload(
        oauth_payload,
        user,
...
@freeze_time("2019-03-18 12:00:00")
@patch("weenspace.plugins.openid_connect.plugin.send_user_event")
@patch("weenspace.plugins.openid_connect.utils.cache.set")
@patch("weenspace.plugins.openid_connect.utils.cache.get")
@pytest.mark.vcr
...
def test_external_obtain_access_tokens_get_or_create_user_from_payload_raises_error(
    ...
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.plugin.get_or_create_user_from_payload",
        Mock(side_effect=AuthenticationError()),
    )
...
def test_validate_plugin_configuration(plugin_configuration, openid_plugin):
    configuration = plugin_configuration(
        ...
        client_secret="c_secret",
        enable_refresh_token=True,
        oauth_authorization_url="http://weenspace.com/auth",
        oauth_token_url="http://weenspace.com/token",
        json_web_key_set_url="http://weenspace.com/jwks",
    )
    conf = PluginConfiguration(active=True, configuration=configuration)
    plugin = openid_plugin()
    plugin.validate_plugin_configuration(conf)
...
def test_authenticate_user(openid_plugin, id_payload, customer_user, monkeypatch, rf):
    ...
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.plugin.get_token_from_request", lambda _: token
    )
...
def test_authenticate_user_with_access_token(
    ...
    plugin = openid_plugin(
        ...
        json_web_key_set_url="https://weenspace.com/.well-known/jwks.json",
        user_info_url="https://weenspace.com/userinfo",
    ...
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.plugin.get_token_from_request",
        lambda _: "OAuth_access_token",
    )

    # decode access token returns payload when access token is in JWT format
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.utils.decode_access_token", lambda x, y: None
    )
    # mock request to api to fetch user info details
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.utils.get_user_info",
        lambda x, z: user_info_response,
    )
    # mock cache used for caching user info details
    monkeypatch.setattr("weenspace.plugins.openid_connect.utils.cache.set", Mock())
...
def test_authenticate_user_with_access_token_unable_to_fetch_user_info(
    ...
    # mock request to api to fetch user info details
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.utils.get_user_info", lambda x, z: None
    )
    # mock cache used for caching user info details
    monkeypatch.setattr("weenspace.plugins.openid_connect.utils.cache.set", Mock())
...
def test_authenticate_user_with_jwt_access_token(
    ...
    # decode access token returns payload when access token is in JWT format
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.utils.decode_access_token",
...
@freeze_time("2021-03-08 12:00:00")
@patch("weenspace.plugins.openid_connect.utils.send_user_event")
...
def test_authenticate_user_get_user_from_oauth_access_token_raises_authentication_error(
    ...
    # mock request to api to fetch user info details
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.plugin.get_user_from_oauth_access_token",
        Mock(side_effect=AuthenticationError()),
    )
...
def test_authenticate_user_get_user_from_access_payload_decode_error(
    ...
    # given
    plugin = openid_plugin(
        oauth_authorization_url="https://weenspace.com/auth",
        oauth_token_url="https://weenspace.com/token",
        json_web_key_set_url="https://weenspace.com/.well-known/jwks.json",
        user_info_url="https://weenspace.com/userinfo",
        use_oauth_scope_permissions=True,
    )
    ...
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.plugin.is_owner_of_token_valid",
        Mock(return_value=True),
    )
    # mock request to api to fetch user info details
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.plugin.get_user_from_access_payload",
...
def test_authenticate_user_get_user_from_access_payload_raises_invalid_token_error(
    ...
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.plugin.jwt_decode",
...
def test_authenticate_user_with_jwt_access_token_unable_to_fetch_user_info(
    ...
    # mock request to api to fetch user info details
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.utils.get_user_info", Mock(return_value=None)
    )
    # mock cache used for caching user info details
    monkeypatch.setattr("weenspace.plugins.openid_connect.utils.cache.set", Mock())
...
def test_authenticate_user_unable_to_decode_token(openid_plugin, monkeypatch, rf):
    plugin = openid_plugin(user_info_url="")
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.plugin.get_token_from_request", lambda _: "ABC"
    )
    user = plugin.authenticate_user(rf.request(), None)
    assert user is None
