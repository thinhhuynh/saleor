...
@pytest.mark.vcr
@mock.patch("weenspace.plugins.openid_connect.utils.cache.set")
def test_fetch_jwks(mocked_cache_set):
    jwks_url = "https://weenspace.com/.well-known/jwks.json"
...
def test_get_or_create_user_from_token_missing_email(id_payload):
    ...
    with pytest.raises(AuthenticationError):
        get_or_create_user_from_payload(id_payload, "https://weenspace.com/oauth")
...
def test_create_tokens_from_oauth_payload(monkeypatch, id_token, id_payload):
    ...
    user, _, _ = get_or_create_user_from_payload(id_payload, "https://weenspace.com/oauth")
...
def test_get_saleor_permissions_from_scope():
    auth_payload = {
        ...
        "scope": (
            ...
            "weenspace:non_existing weenspace weenspace: weenspace:manage_users"
        ),
    }
...
def test_get_user_info_raises_decode_error(monkeypatch):
    ...
    user_info = get_user_info("https://weenspace.com/userinfo", "access_token")
...
@mock.patch("weenspace.plugins.openid_connect.utils.cache.set")
@mock.patch("weenspace.plugins.openid_connect.utils.cache.get")
...
@patch("weenspace.plugins.manager.PluginsManager.customer_created")
def test_jwt_token_without_expiration_claim(
    mocked_customer_created_webhook, monkeypatch, decoded_access_token
):
    # given
    monkeypatch.setattr(
        "weenspace.plugins.openid_connect.utils.get_user_info_from_cache_or_fetch",
        lambda *args, **kwargs: {
...
@patch("weenspace.plugins.openid_connect.utils.match_orders_with_new_user")
@patch("weenspace.plugins.openid_connect.utils.logger")
