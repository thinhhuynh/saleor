...
@override_settings(PLUGINS=["weenspace.plugins.avatax.plugin.AvataxPlugin"])
...
def test_calculate_checkout_total_uses_default_calculation(
    ...
    plugin_configuration()
    monkeypatch.setattr(
        "weenspace.plugins.avatax.plugin.get_cached_tax_codes_or_fetch",
...
def test_calculate_checkout_total(
    ...
    monkeypatch.setattr(
        "weenspace.plugins.avatax.plugin.AvataxPlugin._skip_plugin", lambda *_: False
    )
    manager = get_plugins_manager(allow_replica=False)
    checkout_with_item.shipping_address = ship_to_pl_address
    checkout_with_item.save()
...
def test_get_cached_tax_codes_or_fetch(monkeypatch, avatax_config):
    monkeypatch.setattr("weenspace.plugins.avatax.cache.get", lambda x, y: {})
...
def test_get_checkout_line_tax_rate_error_in_response(
    ...
    plugin_configuration()
    monkeypatch.setattr(
        "weenspace.plugins.avatax.plugin.get_checkout_tax_data",
...
def test_get_order_line_tax_rate_error_in_response(
    ...
    plugin_configuration()
    monkeypatch.setattr(
        "weenspace.plugins.avatax.plugin.get_order_tax_data",
        lambda *_: {"error": "Example error"},
    )
    unit_price = TaxedMoney(Money(12, "USD"), Money(15, "USD"))
...
def test_get_checkout_shipping_tax_rate_skip_plugin(
    ...
    plugin_configuration()
    monkeypatch.setattr(
        "weenspace.plugins.avatax.plugin.AvataxPlugin._skip_plugin",
        lambda *_: True,
    )
    shipping_price = TaxedMoney(Money(12, "USD"), Money(15, "USD"))
...
def test_get_plugin_configuration(settings, channel_USD):
    settings.PLUGINS = ["weenspace.plugins.avatax.plugin.AvataxPlugin"]
    manager = get_plugins_manager(allow_replica=False)
...
@patch("weenspace.plugins.avatax.plugin.api_get_request")
...
@patch("weenspace.plugins.avatax.plugin.api_post_request_task.delay")
@override_settings(PLUGINS=["weenspace.plugins.avatax.plugin.AvataxPlugin"])
...
@patch("weenspace.plugins.avatax.get_order_request_data")
@patch("weenspace.plugins.avatax.get_cached_response_or_fetch")
...
@pytest.mark.vcr
@patch("weenspace.plugins.avatax.cache.set")
...
@patch("weenspace.plugins.avatax.plugin.get_checkout_tax_data")
...
@patch("weenspace.plugins.avatax.plugin.get_order_tax_data")
@override_settings(PLUGINS=["weenspace.plugins.avatax.plugin.AvataxPlugin"])
