...
from weenspace.payment.utils import price_to_minor_unit
...
@patch(
    "weenspace.payment.gateways.stripe.stripe_api.stripe.WebhookEndpoint",
)
...
def test_subscribe_webhook_returns_webhook_object(mocked_webhook, channel_USD):
    api_key = "api_key"
    expected_url = (
        "http://mirumee.com/plugins/channel/main/weenspace.payments.stripe/webhooks/"
    )
...
@patch(
    "weenspace.payment.gateways.stripe.stripe_api.stripe.PaymentIntent",
)
...
@patch(
    "weenspace.payment.gateways.stripe.stripe_api.stripe.PaymentMethod",
)
...
@patch(
    "weenspace.payment.gateways.stripe.stripe_api.stripe.Refund",
)
...
@patch(
    "weenspace.payment.gateways.stripe.stripe_api.stripe.Customer",
)
