...
@patch("weenspace.webhook.transport.synchronous.transport.send_webhook_request_sync")
...
def test_get_discount_for_checkout_value_entire_order_voucher(
    ...
    monkeypatch.setattr(
        "weenspace.checkout.base_calculations.base_checkout_subtotal",
...
def test_get_discount_for_checkout_value_specific_product_voucher(
    ...
    checkout = Mock(spec=checkout_with_items, channel=channel_USD)
    monkeypatch.setattr(
        "weenspace.checkout.utils.get_base_lines_prices",
...
@patch("weenspace.discount.utils.validate_voucher")
...
def test_get_discount_for_checkout_specific_products_voucher_not_applicable(
    ...
    monkeypatch.setattr(
        "weenspace.checkout.utils.get_prices_of_discounted_specific_product",
        lambda checkout, product: [],
    )
    monkeypatch.setattr(
        "weenspace.checkout.calculations.checkout_shipping_price",
...
def test_get_discount_for_checkout_shipping_voucher(
    ...
    monkeypatch.setattr(
        "weenspace.checkout.utils.is_shipping_required", lambda lines: True
    )
    checkout = Mock(
        spec=Checkout,
...
def test_get_discount_for_checkout_shipping_voucher_limited_countries(
    ...
    monkeypatch.setattr(
        "weenspace.checkout.utils.base_calculations.base_checkout_subtotal",
        lambda *args: subtotal,
    )
    checkout = Mock(
...
def test_get_discount_for_checkout_shipping_voucher_not_applicable(
    ...
    monkeypatch.setattr(
        "weenspace.checkout.utils.is_shipping_required", lambda lines: is_shipping_required
    )
    checkout = Mock(
