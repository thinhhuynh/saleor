...
@patch("weenspace.checkout.calculations._apply_tax_data")
...
@patch(
    "weenspace.checkout.calculations.update_checkout_prices_with_flat_rates",
...
def test_fetch_checkout_prices_when_tax_exemption_and_include_taxes_in_prices(
    ...
    """
    # given
    settings.PLUGINS = ["weenspace.plugins.tests.sample_plugins.PluginSample"]
    manager = get_plugins_manager(allow_replica=False)
...
@freeze_time()
@patch("weenspace.plugins.manager.PluginsManager.calculate_checkout_total")
@patch("weenspace.plugins.manager.PluginsManager.get_taxes_for_checkout")
@override_settings(PLUGINS=["weenspace.plugins.tests.sample_plugins.PluginSample"])
