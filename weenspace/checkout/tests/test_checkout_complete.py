...
@mock.patch("weenspace.plugins.manager.PluginsManager.notify")
...
@mock.patch("weenspace.giftcard.utils.send_gift_card_notification")
@pytest.mark.parametrize("is_anonymous_user", [True, False])
...
@mock.patch("weenspace.checkout.complete_checkout._create_order")
@mock.patch("weenspace.checkout.complete_checkout._process_payment")
...
def test_complete_checkout_checkout_was_deleted_before_completing(
    ...
    with before_after.after(
        "weenspace.checkout.complete_checkout._process_payment", convert_checkout_to_order
    ):
...
@mock.patch("weenspace.payment.gateway.payment_refund_or_void")
...
@mock.patch("weenspace.checkout.complete_checkout.complete_checkout_with_transaction")
...
@mock.patch("weenspace.checkout.complete_checkout.complete_checkout_with_payment")
...
@mock.patch("weenspace.checkout.calculations.get_tax_calculation_strategy_for_checkout")
@mock.patch("weenspace.checkout.complete_checkout._create_order")
@mock.patch("weenspace.checkout.complete_checkout._process_payment")
def test_complete_checkout_ensure_prices_are_not_recalculated_in_post_payment_part(
    ...
    # when
    with before_after.after(
        "weenspace.checkout.complete_checkout._process_payment", update_price_expiration
    ):
        order, action_required, _ = complete_checkout(
            checkout_info=checkout_info,
            lines=lines,
