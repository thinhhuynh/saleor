...
@patch(
    "weenspace.product.management.commands"
    ".update_all_products_discounted_prices"
    ".update_discounted_prices_for_promotion"
)
@patch(
    "weenspace.product.management.commands.update_all_products_discounted_prices.DISCOUNTED_PRODUCT_BATCH",
    1,
)
...
def test_update_discounted_price_for_promotion_promotion_rule_deleted_in_meantime(
    ...
    with before_after.before(
        "weenspace.product.utils.variant_prices._get_discounted_variants_prices_for_promotions",
...
def test_update_discounted_price_rule_deleted_in_meantime_promotion_listing_exist(
    ...
    # when
    with before_after.before(
        "weenspace.product.utils.variant_prices._update_or_create_listings",
        delete_promotion_rule,
    ):
        update_discounted_prices_for_promotion(
            Product.objects.filter(id__in=[product.id])
